...
function loadCategories() {
    return supabase.from('categories').select('*').order('name').then(function(res) {
...
function loadProducts() {
    return supabase.from('products').select('*, categories(name, type)').order('name').then(function(res) {
        if (res.error) throw res.error;
...
function loadContainers() {
    return supabase.from('containers').select('*').order('name').then(function(res) {
        if (res.error) throw res.error;
...
function loadCustomers() {
    return supabase.from('customers').select('*').order('name').then(function(res) {
        if (res.error) throw res.error;
...
function loadSuppliers() {
    return supabase.from('suppliers').select('*').order('name').then(function(res) {
        if (res.error) throw res.error;
...
function loadTableItems() {
    return supabase.from('table_items').select('*, suppliers(name)').order('name').then(function(res) {
        if (res.error) throw res.error;
...
function loadIngredients() {
    return supabase.from('ingredients').select('*').order('name').then(function(res) {
        if (res.error) throw res.error;
...
function loadPackingRules() {
    return supabase.from('packing_rules').select('*, products(name), containers(name)').order('product_id').then(function(res) {
        if (res.error) throw res.error;
...
function loadOrders() {
    ...
    var query = supabase.from('orders').select('*, customers(name)').order('event_date', { ascending: true });
...
function loadHomeStats() {
    ...
    supabase.from('orders').select('id').eq('event_date', today).then(function(res) {
        var el = document.getElementById('stat-orders-today');
        if (el) el.textContent = (res.data || []).length;
    });
    
    supabase.from('orders').select('id').gte('event_date', weekAgo).then(function(res) {
        var el = document.getElementById('stat-orders-week');
        if (el) el.textContent = (res.data || []).length;
    });
    
    supabase.from('orders').select('total_amount, paid_amount').then(function(res) {
        var orders = res.data || [];
        var pending = 0;
    ...
    supabase.from('orders').select('*, customers(name)').eq('event_date', today).order('delivery_time').then(function(res) {
...
function loadOrdersForReportSelect() {
    var today = new Date().toISOString().split('T')[0];
    supabase.from('orders').select('*, customers(name)').gte('event_date', today).order('event_date').then(function(res) {
...
function loadPaymentsPage() {
    supabase.from('orders').select('*, customers(name)').order('event_date', { ascending: false }).then(function(res) {
        var orders = res.data || [];
...
function deleteCategory(id) {
    ...
    supabase.from('categories').delete().eq('id', id).then(function(res) {
        if (res.error) throw res.error;
...
function saveContainer() {
    ...
    var promise = id 
        ? supabase.from('containers').update(data).eq('id', id)
        : supabase.from('containers').insert([data]);
...
function deleteContainer(id) {
    ...
    supabase.from('containers').delete().eq('id', id).then(function(res) {
        if (res.error) throw res.error;
        showToast('מיכל נמחק');
        loadContainers();
    }).catch(function() { showToast('שגיאה', 'error'); });
...
function deleteSupplier(id) {
    ...
    supabase.from('suppliers').delete().eq('id', id).then(function(res) {
        if (res.error) throw res.error;
        showToast('ספק נמחק');
        loadSuppliers();
    }).catch(function() { showToast('שגיאה', 'error'); });
...
function saveTableItem() {
    ...
    var promise = id 
        ? supabase.from('table_items').update(data).eq('id', id)
        : supabase.from('table_items').insert([data]);
...
function deleteTableItem(id) {
    ...
    supabase.from('table_items').delete().eq('id', id).then(function(res) {
        if (res.error) throw res.error;
        showToast('נמחק');
        loadTableItems();
    }).catch(function() { showToast('שגיאה', 'error'); });
...
function savePackingRule() {
    ...
    var promise = id 
        ? supabase.from('packing_rules').update(data).eq('id', id)
        : supabase.from('packing_rules').insert([data]);
...
function deletePackingRule(id) {
    if (!confirm('למחוק?')) return;
    supabase.from('packing_rules').delete().eq('id', id).then(function(res) {
        if (res.error) throw res.error;
        showToast('נמחק');
        loadPackingRules();
    }).catch(function() { showToast('שגיאה', 'error'); });
...
function saveIngredient() {
    ...
    var promise = id 
        ? supabase.from('ingredients').update(data).eq('id', id)
        : supabase.from('ingredients').insert([data]);
...
function deleteIngredient(id) {
    if (!confirm('למחוק?')) return;
    supabase.from('ingredients').delete().eq('id', id).then(function(res) {
        if (res.error) throw res.error;
        showToast('נמחק');
        loadIngredients();
    }).catch(function() { showToast('שגיאה', 'error'); });
...
function savePayment() {
    ...
    supabase.from('payments').insert([data]).then(function(res) {
        if (res.error) throw res.error;
        return supabase.from('orders').select('paid_amount').eq('id', orderId).single();
    }).then(function(res) {
        var newPaid = (res.data.paid_amount || 0) + amount;
        return supabase.from('orders').update({ paid_amount: newPaid }).eq('id', orderId);
    }).then(function() {
...
function saveOrder() {
    ...
    supabase.from('orders').insert([orderData]).select().single().then(function(res) {
        if (res.error) throw res.error;
        var order = res.data;
        ...
        var promises = [];
        if (orderItems.length > 0) promises.push(supabase.from('order_items').insert(orderItems));
        if (tableItems.length > 0) promises.push(supabase.from('order_table_items').insert(tableItems));
...
function generateReport(reportType) {
    ...
    var query;
    if (orderId) {
        query = supabase.from('orders').select('*, customers(name), order_items(*, products(name, categories(type))), order_table_items(*, table_items(name))').eq('id', orderId);
    } else {
        query = supabase.from('orders').select('*, customers(name), order_items(*, products(name, categories(type))), order_table_items(*, table_items(name))').eq('event_date', reportDate);
    }
